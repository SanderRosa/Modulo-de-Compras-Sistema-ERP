...
// Função auxiliar robusta para ler números inteiros e evitar quebras se o
// usuário digitar letras.
int obterInteiro() {
  int valor;
  // Loop que continua enquanto a leitura falhar (ex: usuário digita "abc" em
  // vez de "123").
  while (!(std::cin >> valor)) {
  ...
  // Ignora qualquer caractere extra que tenha sobrado no buffer (como o
  // 'Enter').
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  return valor;
}
...
double obterDouble() {
  double valor;
  // Mesma lógica do obterInteiro: tenta ler, se falhar, limpa o erro e pede de
  // novo.
  while (!(std::cin >> valor)) {
    std::cin.clear();
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
...
void menuCadastrarFornecedor(ModuloCompras &modulo) {
  ...
  try {
    ...
    int id =
        modulo.adicionarFornecedor(nome, endereco, cnpj, produto, precoProduto);
...
void menuCriarOrdenCompra(ModuloCompras &modulo) {
  ...
  try {
    ...
    int idOrdem = modulo.criarOrdemCompra(idItem, quantidade, valorUnitario,
                                          idFornecedor);
...
void menuInvestigarFornecedor(ModuloCompras &modulo) {
  ...
  std::cout << "   Procurando por: \"" << forn->getNome() << " "
            << forn->getCNPJ() << "\"\n\n";
...
void menuCriarPedidoMaterial(ModuloCompras &modulo) {
  ...
  if (prioridade < 1 || prioridade > 3) {
    ...
    int idPedido =
        modulo.criarPedidoMaterial(idMaterial, quantidade, prioridade);
    if (idPedido > 0) {
...
void menuConsultarSaldoFinanceiro(ModuloCompras &modulo) {
  ...
  std::cout << "\nSALDO DISPONIVEL: R$ " << std::fixed << std::setprecision(2)
            << saldo << "\n";
  std::cout << "\nPressione ENTER para continuar...";
  std::cin.get();
}
...
int main() {
  try {
    ...
    // Loop principal do menu: mantém o programa rodando até o usuário escolher
    // Sair.
    while (rodando) {
      exibirCabecalho();
      exibirMenu();
      ...
      case 1:
        ...
        limparTela(); // Limpa a tela após cada operação para manter a interface
                      // organizada.
        break;
    ...
    // Captura erros críticos não tratados (ex: falha de memória) e exibe antes
    // de fechar.
    std::cout << "\nERRO CRITICO: " << e.what() << "\n";
